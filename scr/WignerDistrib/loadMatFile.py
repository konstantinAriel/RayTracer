    ...
    for Zi in Zin:
        ...
        for Kxi in Kx:
            ...
            for Kzi in Kz:
                ...
                dxi = (Xi - xPointArray[xCellNum])/2
                xKx = Kxi*dxi
                dzi = (Zi - zPointArray[zCellNum])/2
                zKz = Kzi * dzi
                cs = np.cos(xKx+zKz+K0*Lx)
                sn = np.sin(xKx+zKz+K0*Lx)

                SumWxRe[zCellNum, xCellNum] = SumWxRe[zCellNum, xCellNum] + Wxi*cs
                # SumWxIm[xCellNum, zCellNum] = SumWxIm[xCellNum, zCellNum] + Wxi
                SumWzRe[zCellNum, xCellNum ] = SumWzRe[zCellNum, xCellNum] + Wzi*cs
                # SumWzIm[xCellNum, zCellNum] = SumWzIm[xCellNum, zCellNum] + Wzi
...
XMesh, ZMesh = np.meshgrid(xPointArray,zPointArray)
...
for x0 in xPointArray:
    ...
    xIndex = xIndex + 1
# WamplDict = dict(
#                     go.Surface(x=XMesh, y=ZMesh, z=Wampl,
#                     showscale = False,
#                     opacity = 1,
#                     type = 'surface',
#                     surfacecolor = 'blue',
...
for k in range(xLine):
    ...
    yVLineDict.append(np.nan)
    yGLineDict.append(np.nan)
...
xzyVLines = dict(
        go.Surface(x=xVLineDict, y=zVLineDict, z=yVLineDict,
                  mode='lines',
                  name='V lines NET',
                  line=dict(width=Linewidth, color='blue')
                  ))
xzyGLines = dict(
        go.Surface(x=xGLineDict, y=zGLineDict, z = yGLineDict,
                  mode='lines',
                  name='G lines NET',
                  line=dict(width=Linewidth, color='red')
                  ))
...
WxMeshDict = dict(
                    go.Surface(x=XMesh, y=ZMesh, z=SumWxRe,
                    showscale = False,
                    opacity = 1,
                    type = 'surface',
                    surfacecolor = 'blue',
...
WxDataplot.append(xzyVLines)
WxDataplot.append(xzyGLines)

WzMeshDict = dict(
                    go.Surface(x=XMesh, y=ZMesh, z = SumWzRe,
                    showscale = False,
                    opacity = 1,
                    type = 'surface',
                    surfacecolor = 'blue',
...
WzDataplot.append(xzyVLines)
WzDataplot.append(xzyGLines)
...
layout1 = go.Layout(width=1920, height=1200,
                   autosize=True,
                   margin=dict(
                       autoexpand=False),
                   title='Test NET', hovermode='closest',
                   )
layout2 = go.Layout(width=1920, height=1200,
                   autosize=True,
                   margin=dict(
                       autoexpand=False),
                   title='Sum Wx Re', hovermode='closest',
                   )
layout3 = go.Layout(width=1920, height=1200,
                   autosize=True,
                   margin=dict(
                       autoexpand=False),
                   title='Sum Wz Re', hovermode='closest',
                   )
layout4 = go.Layout(width=1920, height=1200,
                   autosize=True,
                   margin=dict(
                       autoexpand=False),
                   title='Wampl Re', hovermode='closest',
                   )
fig = dict(data=dataPlotDict, layout=layout1)
py.offline.plot(fig, filename = 'testNET.html')

fig = dict(data=WxDataplot, layout=layout2)
py.offline.plot(fig, filename = 'Sum Wx Re.html')

fig = dict(data=WzDataplot, layout=layout3)
py.offline.plot(fig, filename = 'Sum Wz Re.html')

fig = dict(data=WAmplDataplot, layout=layout4)
